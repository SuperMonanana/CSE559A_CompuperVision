              ...
              int nRANSAC, double RANSACthresh, CTransform3x3& M)
{
    ...
    vector<int> bestIndexAssociatedInliers;

    for (int i = 0; i < nRANSAC; i++)
    {
        int randomFeature = rand() % matches.size();  
        Feature F1 = f1[matches[randomFeature].id1 - 1];
        Feature F2 = f2[matches[randomFeature].id2 - 1];

        float xTrans = F2.x - F1.x;
        float yTrans = F2.y - F1.y;

        CTransform3x3 transM = CTransform3x3::Translation(xTrans, yTrans);

        // count good matches
        vector<int> inliers;
        int count = countInliers(f1, f2, matches, m, f, transM, RANSACthresh, inliers);
                    ...
                    const vector<int> &inliers, CTransform3x3& M)
{
    ...
    for (int i = 0; i < inliers.size(); i++)
    {
        ...
        Feature F1 = f1[matches[inliers[i]].id1 - 1];
        Feature F2 = f2[matches[inliers[i]].id2 - 1];
